...
@implementation NTESActivityToast
static NTESActivityToast *myActivityToast = nil;
@synthesize activeDelegate;

+ (NTESActivityToast *)sharedActivityToast {
	if (myActivityToast == nil) {
		myActivityToast = [[NTESActivityToast alloc] initWithFrame:CGRectMake(0, 20, SCREEN_W, SCREEN_H-20)];
	}	
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {		
		...
		[button setTitleColor:RGBCOLOR(64,64,64) forState:UIControlStateNormal];
		...
		[button setBackgroundImage:[UIImage imageNamed:@"toast_button.png"] forState:UIControlStateNormal];
