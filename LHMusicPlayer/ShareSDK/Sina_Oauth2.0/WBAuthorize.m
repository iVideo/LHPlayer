...
-(void)beginToRequestAccessTokenWithAuthorizeCode:(NSString *)code{
...
- (void)request:(WBRequest *)theRequest didFinishLoadingWithResult:(id)result
{
    ...
    if ([result isKindOfClass:[NSDictionary class]])
    {
        ...
        if (success && [delegate respondsToSelector:@selector(authorize:didSucceedWithAccessTokenString:)]){
            NSString *tokenString = @"";
            for (NSString *key in [dict allKeys]) {
                tokenString = [tokenString stringByAppendingFormat:@"%@=%@", key, [dict objectForKey:key]];
                tokenString = [tokenString stringByAppendingString:@"&"];
            }
            tokenString = [tokenString stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"&"]];
            [delegate authorize:self didSucceedWithAccessTokenString:tokenString];
        }
    }
