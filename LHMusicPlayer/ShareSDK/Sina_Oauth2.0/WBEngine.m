...
- (void)readAuthorizeDataFromKeychain
{
    ...
    // 如果uid和accessToken为空，检查一下LOGIN_DATA_SINA_KEY
    if (self.userID == nil || [self.userID isEqual:[NSNull null]] || self.accessToken == nil || [self.accessToken isEqual:[NSNull null]]) {
        NSString *token = [userDefault objectForKey:LOGIN_DATA_SINA_KEY];
        if (token && ![token isEqual:[NSNull null]]) {
            [self saveAuthorizedTokenString:token];
        }
    }
}
...
-(void)saveAuthorizedTokenString:(NSString *)token{
    if (token && ![token isEqual:[NSNull null]]) {
        NSArray *tokenArray = [token componentsSeparatedByString:@"&"];
        for (NSString *temp in tokenArray) {
            if ([temp rangeOfString:@"access_token"].length > 0) {
                NSArray *tokenArray = [temp componentsSeparatedByString:@"="];
                if (tokenArray.count > 1) {
                    accessToken = [tokenArray objectAtIndex:1];
                }
            }
        
            if ([temp rangeOfString:@"uid"].length > 0) {
                NSArray *uidArray = [temp componentsSeparatedByString:@"="];
                if (uidArray.count > 1) {
                    userID = [uidArray objectAtIndex:1];
                }
            }
                
            if ([temp rangeOfString:@"expires_in"].length > 0) {
                NSArray *timeArray = [temp componentsSeparatedByString:@"="];
                if (timeArray.count > 1) {
                    expireTime = [[timeArray objectAtIndex:1] intValue]+[[NSDate date] timeIntervalSince1970];
                }
            }
        }
...
-(void)loginWithWebview:(UIWebView*)webView{
    if (webView) {
        [self logIn];
        NSURLRequest *req =[NSURLRequest requestWithURL:[authorize getAuthorizeURL]
...
-(void)authorize:(WBAuthorize *)authorize didSucceedWithAccessTokenString:(NSString *)theAccessToken{
    if (theAccessToken && ![theAccessToken isEqual:[NSNull null]] && delegate && [delegate respondsToSelector:@selector(engineDidLogIn:withTokenString:)]) {
        [delegate engineDidLogIn:self withTokenString:theAccessToken];
    }
}
