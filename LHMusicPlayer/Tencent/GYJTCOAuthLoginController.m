...
@interface GYJTCOAuthLoginController() <UIWebViewDelegate>{
...
@implementation GYJTCOAuthLoginController
    ...
    if (tokenRange.length > 0) {
        ...
        if (2 == [tokenList count]) {
            ...
            if (accessToken.length > 0) {
                [[GYJTencentManager shareInstance] storageAccessToken:accessToken];
                    // 绑定成功，发个消息
        ...
        [[GYJActivityToast sharedActivityToast] stopSpinWithInfo:info withButtonTitle:nil];
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    ...
    [[GYJActivityToast sharedActivityToast] stopSpinWithInfo:@"请求失败" withButtonTitle:nil];
}

- (void)activityWindowButtonClicked:(NSString *)btnTitle{
    browseView.delegate = nil;
    [[GYJActivityToast sharedActivityToast] stopSpinWithInfo:@"授权取消！" withButtonTitle:nil];
    if (oauthDelegate && [oauthDelegate respondsToSelector:@selector(finishedWebRequestWithStatus:)]) {
