//
//  GYJMTencentWeiboViewController.m
//  Magazine
//
...
@interface GYJTencentWeiboViewController ()
{
    BOOL _isImageAnimate;
}
@end

@implementation GYJTencentWeiboViewController
...
- (id)init{
    if (self = [super init]) {
        [[GYJTencentManager shareInstance] setTencentDelegate:self];
    }
    return self;
}
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [[GYJTencentManager shareInstance] cancelRequest];
}
...
- (void)shareButtonPressed {
	...
	if(nil == username || nil == userData || [[GYJTencentManager shareInstance] oauthExpired]) {
            //推出登录界面
        NSString *requestTokenUrl = [[GYJTencentManager shareInstance] requestTencentAuthorizeUrl];
        UIViewController *viewController = [[GYJTCOAuthLoginController alloc] initWithUrlStr:requestTokenUrl];
        ...
        [GYJActivityToast sharedActivityToast].tag = TAG_SHARE_ACTION;
        [[GYJActivityToast sharedActivityToast] startSpinWithInfo:@"发送中" withButtonTitle:@"取消" careLandscope:NO];
...
- (NSString *)loginName{
	...
	if (![[GYJTencentManager shareInstance] oauthExpired] && userData != nil && username != nil) {
        bindString = @"已经认证";
    } else if ([[GYJTencentManager shareInstance] oauthExpired]) {
        bindString = @"尚未认证";
        [[GYJTencentManager shareInstance] removeAccessToken];
    } else {
        bindString = nil;
        [[GYJTencentManager shareInstance] removeAccessToken];
    }
...
- (void)shareMicroBlogWithStatus:(NSString *)status {
    if (hasPhoto) {
        [[GYJTencentManager shareInstance] sendTwitter:status image:sharedOriginImage];
    } else {
        [[GYJTencentManager shareInstance] sendTwitter:status image:nil];
    }
...
- (void)finishSendTwitterWithSuccessStatus:(BOOL)success info:(NSString*)aInfo {
    if (success) {
        [GYJActivityToast sharedActivityToast].activeDelegate = nil;
        [[GYJActivityToast sharedActivityToast] stopSpinWithInfo:@"分享成功！" withButtonTitle:nil];
        [self dismissViewControllerAnimated:YES completion:nil];
    }
    else
    {
        [GYJActivityToast sharedActivityToast].activeDelegate = nil;
        [[GYJActivityToast sharedActivityToast] stopSpinWithInfo:@"分享失败！" withButtonTitle:nil];
        [self dismissViewControllerAnimated:YES completion:nil];
    }
}
